...
[1]	
Notice that here the roles of read and write are reversed again, so in ioctl's read is to send information to the kernel and write is to receive information from 
the kernel.
...
Prev	Home	Next
Talking To Device Files	Up	System Calls
