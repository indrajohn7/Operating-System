...
void link_sil_lib()
{
	...
	/*link SIL lib*/
	sys_qsfp_page_read_sil sys_qsfp_page_read  = (sys_qsfp_page_read_sil)dlsym( lib, "sys_qsfp_page_read" );
	
	sys_port_present_sil sys_port_present  = (sys_port_present_sil)dlsym( lib, "sys_port_present" );
}
#endif
...
//	printf("message_queue_signal_handler \r\n");
	if(mq_receive(my_mq, (char*)&om_ports, sizeof(struct OM_PORTS), NULL) == sizeof(struct OM_PORTS)){
	...
	while (!interrupted) {
		...
		if (rc == ETIMEDOUT) {
			...
			for(iter = 0; iter < MAX_LOCAL_PORTS ; iter++){
				 ...
				 *	*/
				send(clientSocket, &(om_data[iter]), sizeof(struct om_port_data), 0);
...
//	while (message_queue_signal_handler() == 1){  //can call the message queue get here
		...
		worker_thread_dom(-1);
		//send OM data
		send(clientSocket, &(om_data[om_ports.local_ports]), sizeof(struct om_port_data), 0);
...
//			}
		...
		if(rc > 0){	
		 	...
		 	* */
			int status=mq_send(my_mq,(char*)&om_ports, sizeof(struct OM_PORTS),1);
			if(status){
