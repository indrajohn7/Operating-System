...
void create_socket(int* fp1,int* fp2)
{
        ...
        if((rc = setsockopt(sd, SOL_SOCKET, SO_REUSEADDR /*| TCP_NODELAY*/, (char*)&on,sizeof(on))) < 0)
        {
	...
	if((sd2 = accept4(sd, (struct sockaddr *)&their_addr, &sin_size, SOCK_NONBLOCK)) < 0)
	{
...
void call_server_thread()
{
	...
	while(1){
		...
		if(rc > 0){
			...
			if (tanto_optical_monitoring(om_data.port) == 1 && tanto_sfp_optical_monitoring_service(om_data.port) == 1){

				transaction_between_dom_and_FI(1);
#if 0
				om_enabled_port_list[local_port] = (SPTR_PORT_DB(port))->port_config.optical_monitor_interval;
				om_ports.local_port = PORT_TO_LOCAL_PORT(om_data.port);
			...
			int time_interval = om_enabled_port_list[PORT_TO_LOCAL_PORT(om_data.port)];
		}		
	//	sleep(time_interval);
