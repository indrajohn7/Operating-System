...
void call_client_thread()
{
	...
	while(1){
		 ...
		 * */
		char temp[1024];
		int status=mq_receive(my_mq,(char*)temp,sizeof(temp),NULL);
		if(status){
			printf("Daemon Client recieved the message queue from the server\n");
		}
		/*END*/
		
		/*	Perform the DOM operation and populate the global data structures required 
		 *	inside the daemon and send back to FI through the same socket fd send operation 
		 *	and once FI gets the info, it copies the same.
		 *
		 *	FI reverted back with the data structures(PORT_DB/global data) to DOM process and once it recieves it
		 *	then goes for a sleep. in the next wake up cycle it opeartes again and send back data.
		 * */
	...
	//	nBytes = recvfrom(clientSocket,buffer,1024,0,NULL, NULL);
		...
		semaphore=1;
		sleep(3); //This sleep will be on OM time interval
	}
...
//			}
		 ...
		 *It will recieve operatable datas from FI.
		 * */
		int status=mq_send(my_mq,(const char*)&buffer[0],sizeof(buffer),1);
		if(status){
			printf("Daemon Server sends the recieved message to Daemon client message queue \n");
		}
		/*END*/
